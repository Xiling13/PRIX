...
1. **找赛事**  
   保持在 **赛事**。用上方分类标签筛选，或按 `⌘K` / `Ctrl+K` 搜索名称、国家、标签（如 `TDC`、海报、`Free`）。
...
3. **加入备赛**  
   在行上点 **加入备赛**，或在面板里点 **加入备赛**。顶部切换到 **看板**，把状态从 感兴趣 → 备稿中 → 已提交 → 入围 → 获奖 往后推。卡片可拖到其他列，不需要时可以移除。可用分类标签筛选（进入看板时自动回到 **全部**），搜索栏 / `⌘K` / `Ctrl+K` 同样适用于备赛列表。看板页支持 **导出** / **导入** 备份进度与自定义赛事。

4. **导出辅助**  
   面板里：**加入日历** 下载 `.ics`（用 Apple / Google 日历打开即可）；**提交资料** 下载该赛事的清单与文件夹结构。

5. **补充未收录赛事（可选）**  
   点 **添加比赛**，填表保存后会自动加入备赛列表；若希望进入公共库，再 **复制 JSON** 或 **打开 GitHub Issue**。

提示：右上角 EN / 汉 可切换语言。搜索在 **赛事** 和 **看板** 均可用。进度与自定义赛事只存在本机浏览器（`localStorage`），清除网站数据会丢失。已截止赛事可能出现在归档区，并带有下一届周期提示。

### 如何贡献新赛事

1. 应用内 **添加比赛** → 复制 JSON / 打开 GitHub Issue；或直接编辑 `src/data/competitions.json`。
